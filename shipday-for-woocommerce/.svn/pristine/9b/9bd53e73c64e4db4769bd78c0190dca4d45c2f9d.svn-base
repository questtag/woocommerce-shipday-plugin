			...
			function general_admin_notice() {
				...
				/* if ( $pagenow == 'options-general.php' ) { */
				 echo "<div class='notice notice-warning is-dismissible'>
					 <p>Your Shipday API Field is blank.</p>
				 </div>";
				/* } */
			}
...
echo 'Shipping cost issss: ' . ($flat_rate_settings['cost_per_order'] + $fee); */
			...
			if ($order->has_shipping_address()){
				echo '<h1>has shipping address</h1>';
...
/* 	}  // end check rest api
		...
		function general_admin_rest_api_notice() {
		...
		add_action( 'admin_notices', 'general_admin_rest_api_notice' );
	} */
}
