    ...
    function update_block_order_meta( $order, $request ) {
        ...
        if ( $settings['enable_delivery_date'] && ( !$settings['enable_delivery_option'] || $data['order_type'] === 'Delivery' ) && !empty( $data['delivery_date'] ) ) {
            if ( $hpos ) {
                ...
                update_post_meta( $order_id, '_shipday_delivery_date', sanitize_text_field( $data['delivery_date'] ) );
            }
        }
        ...
        if ( $settings['enable_delivery_time'] && ( !$settings['enable_delivery_option'] || $data['order_type'] === 'Delivery' ) && !empty( $data['delivery_time'] ) ) {
            if ( $hpos ) {
                ...
                update_post_meta( $order_id, '_shipday_delivery_time', sanitize_text_field( $data['delivery_time'] ) );
            }
        }
        ...
        if ( $settings['enable_pickup_date'] &&  $data['order_type'] === 'Pickup' && !empty( $data['pickup_date'] ) ) {
            if ( $hpos ) {
                ...
                update_post_meta( $order_id, '_shipday_delivery_date', sanitize_text_field( $data['pickup_date'] ) );
            }
        }
